# Ulises Antonio Ponce Pretelin

**Physical Therapist & Clinical AI Engineer** · Xalapa, Veracruz, Mexico

Ulises Antonio Ponce Pretelin is a physical therapist and clinical AI engineer in Xalapa, Mexico. He created **PoncePretelin**, a clinical manager with referenced evidence, openFDA/RxNav integrations and AI support modules for physical therapy — not a generic EMR.

I design clinical systems where evidence, safety integrations and AI support meet real rehabilitation workflows — not empty EMR forms.

## Featured project — PoncePretelin

Clinical manager for physical therapy (v1 in active development). Free clinical core; optional AI add-on.

| Engine | What it does |
|--------|----------------|
| **Clinical core** | Dashboard, schedule with public booking & cancellations, teleconsult, patients, episodes and clinic workflow |
| **Safety & validation** | openFDA + RxNav integrations for device alerts and medication context |
| **Terminology** | IASP / WHO ICF clinical language corrector in notes and reports |
| **AI support** | Live SOAP organizer, evidence atlas and imaging with explicit non-diagnostic limits — support, not substitute |

- [Live demo](https://www.poncepretelin.com) · `demo@poncepretelin.app` / `PoncePretelinDemo2026!`
- [Portfolio](https://ulisesponcepretelin.github.io/)

## Dual stack

**Tech:** TypeScript · Next.js 15 · React 19 · Python · FastAPI · PostgreSQL · Docker · Vercel + Railway

**Clinical:** IASP · WHO ICF · openFDA · RxNav · EBP scales (NDI, ODI, Berg, TUG) · referenced Sn/Sp

## Connect

- [LinkedIn](https://www.linkedin.com/in/ulises-antonio-ponce-pretelin-3a1a8a13a)
- [Portfolio & CV](https://ulisesponcepretelin.github.io/)
- [Email](mailto:poncepretelin@gmail.com)
